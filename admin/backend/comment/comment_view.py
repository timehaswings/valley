...
class CommentAPIView(APIView):
    ...
    def patch(self, request, format=None):
        ...
        try:
            ...
            obj = Comment.objects.filter(id=data.get('id')).first()
            if obj:
                try:
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if 'id' in params.keys():
                obj = Comment.objects.filter(id=params.get('id')).first()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = CommentSerializer(obj).data
                return Response(result, status.HTTP_200_OK)
            else:
                ...
                rows = Comment.objects.filter(**filters).order_by(*sort_list)[startIndex:endIndex]
                rows = CommentSerializer(rows, many=True).data
                total = Comment.objects.filter(**filters).count()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = {}
