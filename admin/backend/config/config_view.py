...
class ConfigAPIView(APIView):
    ...
    def patch(self, request, format=None):
        ...
        try:
            ...
            obj = Config.objects.filter(id=data.get('id')).first()
            if obj:
                try:
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if 'id' in params.keys():
                obj = Config.objects.filter(id=params.get('id')).first()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = ConfigSerializer(obj).data
                return Response(result, status.HTTP_200_OK)
            else:
                ...
                filters = {'is_delete': 0}
                rows = Config.objects.filter(**filters).order_by(*sort_list)[startIndex:endIndex]
                rows = ConfigSerializer(rows, many=True).data
                total = Config.objects.filter(**filters).count()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = {}
