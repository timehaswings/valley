...
from backend.serializers import TagsSerializer
...
class TagsAPIView(APIView):
    ...
    def patch(self, request, format=None):
        ...
        try:
            ...
            obj = Tags.objects.filter(id=data.get('id')).first()
            if obj:
                try:
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if 'id' in params.keys():
                obj = Tags.objects.filter(id=params.get('id')).first()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = TagsSerializer(obj).data
                return Response(result, status.HTTP_200_OK)
            else:
                ...
                rows = Tags.objects.filter(**filters).order_by(*sort_list)[startIndex:endIndex]
                rows = TagsSerializer(rows, many=True).data
                total = Tags.objects.filter(**filters).count()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = {}
