...
from backend.models import Discover
from backend.serializers import DiscoverSerializer
...
class DiscoverAPIView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            try:
                serializer = DiscoverSerializer(data=data)
    ...
    def patch(self, request, format=None):
        ...
        try:
            ...
            obj = Discover.objects.filter(id=data.get('id')).first()
            if obj:
                try:
                    serializer = DiscoverSerializer(obj, data=data, partial=True)
                    serializer.is_valid(raise_exception=True)
                    serializer.save()
                except Exception as e:
                    result['msg'] = '操作失败：%s' % e
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if 'id' in params.keys():
                obj = Discover.objects.filter(id=params.get('id')).first()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = DiscoverSerializer(obj).data
                return Response(result, status.HTTP_200_OK)
            else:
                ...
                rows = Discover.objects.filter(**filters).order_by(*sort_list)[startIndex:endIndex]
                rows = DiscoverSerializer(rows, many=True).data
                total = Discover.objects.filter(**filters).count()
                result['msg'] = '获取成功'
                result['success'] = True
                result['data'] = {}
